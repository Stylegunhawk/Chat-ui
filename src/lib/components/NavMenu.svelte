...
>
	...
	{#if user?.username || user?.email || !page.data.loginEnabled}
		<button
	...
	{#if user?.username || user?.email}
		<form method="POST" action="{base}/logout">
			<button
				type="submit"
				class="flex h-9 w-full flex-none items-center gap-1.5 rounded-lg pl-2.5 pr-2 text-red-500 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20"
			>
