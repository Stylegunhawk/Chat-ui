...
<div
	class="my-4 flex flex-col gap-2 rounded-xl border border-gray-200 bg-gray-50/50 p-3 dark:border-gray-800 dark:bg-gray-900/30"
>
	<button
		class="flex w-full items-center justify-between text-sm font-medium text-gray-700 transition-colors hover:text-black dark:text-gray-300 dark:hover:text-white"
	...
	{#if expanded}
		<div class="mt-2 grid gap-2 sm:grid-cols-2" transition:slide>
			{#each chunks as chunk}
				<div
					class="flex flex-col gap-1 rounded-lg border border-gray-200 bg-white p-2 text-xs shadow-sm dark:border-gray-700 dark:bg-gray-800"
				>
					<div class="flex items-center justify-between">
						...
						<span
							class={`rounded px-1.5 py-0.5 text-[10px] font-medium ${getRelevanceColor(chunk.similarity)}`}
						>
					...
					<!-- Snippet Preview -->
					<div
						class="mt-1 max-h-20 overflow-hidden font-mono text-[10px] text-gray-500 opacity-80 dark:text-gray-400"
					>
