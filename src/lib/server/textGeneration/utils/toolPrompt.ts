...
export function buildToolPreprompt(tools: OpenAiTool[], ragEnabled = true): string {
	...
	return [
		...
		`- generate_artifact: Use whenever you produce output the user should SEE rendered — not just as a code block to read. Call this for HTML apps, React components, SVG graphics, Mermaid diagrams, JSON data, Markdown documents, CSV tables, or Flutter widgets. The artifact panel opens automatically. Parameters: type (one of: text/html | text/x-react | image/svg+xml | text/x-mermaid | application/json | text/markdown | text/csv | text/x-flutter), title (short label, 2-5 words), content (full string — complete and self-contained). For HTML: include <!DOCTYPE html>. For React (text/x-react): write JSX with a top-level App function — no imports needed (React, useState, useEffect etc. are available as globals; Tailwind CSS is pre-loaded). For Mermaid: start with graph/sequenceDiagram/etc. For Flutter: write a complete widget tree (shown as source with DartPad link). Do NOT call this for code snippets meant to be read — only for content meant to be rendered or inspected.`,
