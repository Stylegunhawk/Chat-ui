...
<div class="fixed inset-0 flex items-center justify-center z-50 devforge-backdrop backdrop-blur-sm">
	<div class="devforge-glass-panel {uiState === 'auth' ? 'expanded' : ''}" id="main-panel">
		...
		<!-- STEP 2: Auth View -->
		<div class="devforge-view {uiState === 'auth' ? 'active' : 'hidden-down'}" id="view-dashboard">
			<h2 class="devforge-auth-header">Join DevForge</h2>
			
			<div class="devforge-auth-buttons">
				<!-- Google Button -->
				<button class="devforge-auth-btn devforge-google-btn" onclick={() => { window.location.href = `${base}/login`; }}>
				...
				<!-- Signup Button -->
				<button class="devforge-auth-btn devforge-email-btn" onclick={() => { window.location.href = `${base}/login`; }}>
					Sign up with Email
				</button>
			</div>

			<div class="mt-[30px] text-[0.85rem] devforge-muted">
...
{#if uiState === "chat"}
	{#if hasModels}
		...
		/>
	{:else}
		<div class="mx-auto my-20 max-w-xl rounded-xl border p-6 text-center dark:border-gray-700 relative z-10 bg-white dark:bg-gray-900">
